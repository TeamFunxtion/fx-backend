...
public class ProductService {
    ...
    @Transactional
    public RsData<ProductCreateResponse> createProduct(ProductCreateRequest productCreateRequest, MultipartFile[] multipartFiles) throws Exception {
    ...
    @Transactional
    public RsData<ProductUpdateResponse> updateProduct(ProductUpdateRequest productUpdateRequest, MultipartFile[] multipartFiles) throws Exception {
    ...
    public RsData<ProductUpdateResponse> changeProductStatus(ProductUpdateRequest productUpdateRequest) throws Exception {
        Optional<Product> optionalProduct = productRepository.findById(productUpdateRequest.getProductId());
        System.out.println(optionalProduct.isEmpty());
    ...
    @Transactional
    public Page<ProductDto> searchByKeyword(String keyword, Pageable pageable) throws Exception {
        Page<ProductDto> list = productRepository.findByProductTitleContainingAndStatusTypeId(keyword, "ST01", pageable)
                .map(ProductDto::new);
        return list;
    }

    public Page<ProductDto> searchByCategory(String category, Pageable pageable) throws Exception {
        Page<ProductDto> list = productRepository.findByCategoryIdAndStatusTypeId(category, "ST01", pageable)
                .map(ProductDto::new);
        return list;
    }

    public Page<ProductDto> getMyProducts(Long userId, String statusTypeId, Pageable pageable) throws Exception {
    ...
    public RsData<List<ProductDto>> getProductList() throws Exception {
    ...
    public RsData<ProductDetailResponse> getProductDetail(Long productId, Long userId) throws Exception {
    ...
    @Transactional
    public int increaseViews(Long id) throws Exception {
        return productRepository.increaseViews(id);
    }

    public boolean updateFavorite(Long userId, Long productId) throws Exception {
    ...
    public Page<ProductDto> getAuctionProducts(Long userId, String statusTypeId, Pageable pageable) throws Exception {
        Optional<Member> member = memberRepository.findById(userId);
        if (member.isEmpty()) {
    ...
    public Page<ProductDto> getBidProducts(Long userId, Pageable pageable) throws Exception {
        Optional<Member> member = memberRepository.findById(userId);
        if (member.isEmpty()) {
            throw new IllegalArgumentException("유저가 존재하지 않습니다.");
        }
